...
class KnowledgeBaseAdmin(admin.ModelAdmin):
    ...
    def display_answer(self, obj):
        ...
        truncated = (string[:char_limit] + "...") if char_limit<len(string) else string
        return truncated
    ...
    def update_embedding(self, request, queryset):
        ...
        for kb in queryset:
            embedding = transformer.generate_embeggings([kb.question,])
            kb.set_embedding(embedding)
            kb.save()
        self.message_user(request, "Selected questions' embeddings have been updated.", messages.SUCCESS)
